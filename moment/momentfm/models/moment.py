...
from moment.momentfm.models.prompt import Prompt
...
class MOMENT(nn.Module):
    def __init__(self, config: Namespace | dict, **kwargs: dict):
        ...
        self.head_reconstruct = self._get_head_reconstruct()
        ...
        print(f'self.freeze_head:{self.freeze_head}')
    ...
    def _get_head_reconstruct(self) -> nn.Module:
        return PretrainHead(
                self.config.d_model,
                self.config.patch_len,
                # self.config.getattr("dropout", 0.1),
